...
# Change the path to where you wish to save the weights file
path = r"C:\Users\Fatwir SM\Desktop\Final_Code_GITLAB\BP Estimation\bp-estimation-mimic3-master\models\\"

# Uncomment line 130 to use multiple GPUs

# Defining the Spectrogram layer
...
def raw_signals_deep_ResNet(input_shape, num_channels):
   ...
   model = Model(inputs=inputs, outputs=output)
    ...
    # If there are multiple GPUs, please uncomment the below line
...
def one_channel_resnet(input_shape,num_filters=16,num_res_blocks = 5,cnn_per_res = 3,
...
def one_channel_resnet_2D(input_shape, input_layer, num_filters=16,num_res_blocks = 5,cnn_per_res = 3,
...
def spectro_layer_mid(input_x,sampling_rate, ndft=0, num_classes=8):
    ...
    x = Normalization2D(str_axis='batch')(x)
    channel_resnet_input,channel_resnet_out= one_channel_resnet_2D((625, 1), x, num_filters=64,
...
class custom_callback(keras.callbacks.Callback):
    ...
    # Save the model only if its performance is upto the mark
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if(logs.get('val_mean_absolute_error') < self.best):
            ...
            self.model.save_weights(path + self.model_name + "_weights.h5")
            
        else:
            print("val_mean_absolute_error has not improved from " + str(self.best) + "...")    
            print("val_mean_absolute_error is " + str(logs.get('val_mean_absolute_error')) + "...")
